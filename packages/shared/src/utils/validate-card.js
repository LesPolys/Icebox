...
export function validateCard(card) {
    ...
    if (c.cost != null && typeof c.cost === "object") {
        validateResourceCost(c.cost, "cost", errors);
    }
    // Faction icons
    if (!Array.isArray(c.factionIcons)) {
        ...
        for (const icon of c.factionIcons) {
            if (!ALL_FACTION_IDS.includes(icon)) {
    ...
    if (c.aging == null || typeof c.aging !== "object") {
        ...
        if (!validDeathOutcomes.includes(aging.onDeath)) {
...
function validateResourceCost(cost, prefix, errors) {
    for (const key of ["matter", "energy", "data", "influence"]) {
        if (cost[key] != null && (typeof cost[key] !== "number" || cost[key] < 0)) {
            errors.push({ field: `${prefix}.${key}`, message: `${key} must be a non-negative number` });
        }
    }
}
//# sourceMappingURL=validate-card.js.map